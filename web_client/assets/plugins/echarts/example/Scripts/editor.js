function initEditor() {
	...
	gb.editor.getSession().setUseWorker(false),
...
function setThemeButton(e) {
	$("#theme-btn").html("ECharts-" + e + ' <span class="caret"></span>'),
	$("#theme-btn").val(e),
	disposeAndRun()
}
function setEchartsVersionButton(e) {
	$("#echarts-version-btn").html("ECharts-" + e + ' <span class="caret"></span>'),
	$("#echarts-version-btn").val(e),
...
function setSplitPosition(e) {
	...
	var t = 100 * e;
	$("#code-container").css("width", t + "%"),
	$(".right-container").css("width", 100 - t + "%").css("left", t + "%"),
	$("#h-handler").css("left", t + "%"),
	gb.chart && gb.chart.resize()
}
...
function fork() {
	if (IS_STATIC)
		return void console.warn("静态版无法使用该功能！");
	var e = getCid(),
	t = getVersion();
...
function localSave() {
	...
	if (window.localStorage && e !== gb.loadedCode)
		try {
...
function localLoad() {
	try {
		...
		e && (gb.editor.setValue(e, -1), log("读取本地缓存成功"))
	} catch (t) {
		console.error(t)
	}
}
...
function load() {
	if (IS_STATIC)
		return void(configs.c && $.ajax("./data/" + configs.c + ".js", {
				dataType : "text",
				success : function (e) {
					gb.loadedCode = e,
	...
	"new" !== e ? $.get("http://" + window.location.host + "/chart/find/" + e, {
		...
		gb.loadedCode = e.code,
		gb.editor.setValue(e.code),
		e.theme && setThemeButton(e.theme),
		setEchartsVersionButton(e.echartsVersion),
		run(),
...
function log(e, t) {
	...
	$("#code-info").html('<span class="code-info-time">' + n + '</span><span class="code-info-type-' + t + '">' + e + "</span>")
}
function formatTime(e) {
	for (var t = [e.getHours(), e.getMinutes(), e.getSeconds()], n = "", o = 0, i = t.length; i > o; ++o)
		n += (t[o] < 10 ? "0" : "") + t[o], i - 1 > o && (n += ":");
	return n
}
function getVersion() {
...
function downloadExample() {
	...
	n.download = configs.c + ".html",
	n.click()
}
var IS_STATIC = !0, configs = {};
...
var appEnv = {}, gui, run = function (ignoreOptionNotChange) {
	if (hasEditorError())
		return void log("编辑器内容有误！", "error");
	localSave(),
	clearTimeout(appEnv.timeTicket),
	clearInterval(appEnv.timeTicket),
	appEnv.config = null;
	try {
				...
				if ("onChange" !== name && "onFinishChange" !== name) {
					...
					if (configParameters[name] && (configParameters[name].options ? controller = gui.add(app.config, name, configParameters[name].options) : null != configParameters[name].min && (controller = gui.add(app.config, name, configParameters[name].min, configParameters[name].max))), "string" == typeof obj)
						try {
							...
							isColor = !!colorArr,
							isColor && (value = echarts.color.stringify(colorArr, "rgba"))
						} catch (e) {}

					controller || (controller = gui[isColor ? "addColor" : "add"](app.config, name)),
